...
void MoveModel::execute(std::shared_ptr<AbstractController> &facade) {
    Point move(dx, dy, dz);
    Point scale(1, 1, 1);
    Point turn(0, 0, 0);
...
void ScaleModel::execute(std::shared_ptr<AbstractController> &facade) {
    Point move(0, 0, 0);
...
void RotateModel::execute(std::shared_ptr<AbstractController> &facade) {
    Point move(0, 0, 0);
    Point scale(1, 1, 1);
    Point turn(ox, oy, oz);
...
void ReformModel::execute(std::shared_ptr<AbstractController> &facade) {
    facade->reform_model(model_numb, move, scale, turn);
}

LoadModel::LoadModel(std::string fname) : fname(std::move(fname)) {}

void LoadModel::execute(std::shared_ptr<AbstractController> &facade) {
    facade->load_model(fname);
}

AddModel::AddModel(std::shared_ptr<Object> model) : model(std::move(model)) {}

void AddModel::execute(std::shared_ptr<AbstractController> &facade) {
    facade->add_model(model);
}

RemoveModel::RemoveModel(const size_t &model_numb) : model_numb(model_numb) {}

void RemoveModel::execute(std::shared_ptr<AbstractController> &facade) {
    facade->remove_model(model_numb);
}

ModelCount::ModelCount(std::shared_ptr<size_t> &count) : _count(count) {}

void ModelCount::execute(std::shared_ptr<AbstractController> &facade) {
    (*_count) = facade->models_count();
}
