...
void MoveModel::execute(std::shared_ptr<Facade> &facade) {
    Point move(dx, dy, dz);
    Point scale(1, 1, 1);
    Point turn(0, 0, 0);
...
void ScaleModel::execute(std::shared_ptr<Facade> &facade) {
    Point move(0, 0, 0);
...
void RotateModel::execute(std::shared_ptr<Facade> &facade) {
    Point move(0, 0, 0);
    Point scale(1, 1, 1);
    Point turn(ox, oy, oz);
...
void AddModel::execute(std::shared_ptr<Facade> &facade) {
